...
try{
	mulai:
	if(!$api->isHTML($api->get_home())) exit("Gagal get_home()");

	$get_captcha = $api->get_captcha();
	if(!$api->isHTML($get_captcha)) exit("Gagal get_captcha()");

	$doc = new DOMDocument();
	$doc->loadHTML($get_captcha);
	$imageTags = $doc->getElementsByTagName('img');
	if(count($imageTags)<1) exit('Gagal mengambil captcha url');
	...
	if(!$api->isJson($get_hasil)) exit('Gagal get_hasil("'.$_GET['nopol'].'","'.$captcha[0].'")');
